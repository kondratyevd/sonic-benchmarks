...
options.register("address", "128.211.142.14", VarParsing.multiplicity.singleton, VarParsing.varType.string, "server address")
#options.register("address", "triton-pytorchtcv3.cms.geddes.rcac.purdue.edu", VarParsing.multiplicity.singleton, VarParsing.varType.string, "server address")
#options.register("address", "triton-2304-py3.cms.geddes.rcac.purdue.edu", VarParsing.multiplicity.singleton, VarParsing.varType.string, "server address")
#options.register("address", "triton-2207-py3-geometric.cms.geddes.rcac.purdue.edu", VarParsing.multiplicity.singleton, VarParsing.varType.string, "server address")
...
modifier_names = []
if options.sonic:
    modifier_names = ["enableSonicTriton"]
    #modifier_names = ["enableSonicTriton", "particleNetSonicTriton"]
    #modifier_names = ["enableSonicTriton", "particleNetSonicTriton", "particleNetPTSonicTriton"]
    #modifier_names = ["enableSonicTriton","deepMETSonicTriton"] #modifier_names = ["allSonicTriton"]
...
for producer in process._Process__producers.values():
    if hasattr(producer,'Client'):
        ...
        if hasattr(producer.Client,'mode'):
            print("do sync mode inference")
            producer.Client.mode = cms.string("Sync")
